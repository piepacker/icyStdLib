...
static LONG NTAPI msw_PageFaultExceptionFilter( EXCEPTION_POINTERS* eps )
{
	...
	if (s_interactive_user_environ && !::IsDebuggerPresent()) {
		...
		auto result = ::MessageBoxA(nullptr,
			"ACCESS VIOLATION (SIGSEGV) has occurred and the emulator has been terminated. "
	...
	if (!prevout || !previn) {
		...
		if (::AttachConsole(ATTACH_PARENT_PROCESS)) {
			...
			if (allow_popups && s_allow_ugly_console) {
				bool alloc_it = 1;
				// this env var only affects behavior when debugger is attached.
				// I'd prefer to hide the console all the time, because msvc's output window is much faster,
				// more robust, with better search tools and highlighting and filters and other productivity goodies.

				if (const char* convar = getenv("EMU_HIDE_UGLY_CONSOLE")) {
					if (convar[0] == '0') {
						alloc_it = ::IsDebuggerPresent();
					}
				}
				if (alloc_it) {
...
void msw_AllocUglyConsole() {
	...
	// Nuke the Close command on the Console, since it prevents us from exiting cleanly
	// (doesn't work in modern windows...)
